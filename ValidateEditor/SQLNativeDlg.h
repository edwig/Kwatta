...
public:
  SQLNativeDlg(CWnd* p_parent);
  virtual ~SQLNativeDlg();
  void     InitTab(ValidateSQL* p_validate,Parameters* p_parameters);
  void     StoreVariables();

// Dialog Data
  enum { IDD = IDD_SQL_NATIVE};
...
public:
  afx_msg void OnBnClickedUseNative();
  afx_msg void OnEnChangeNative();
  afx_msg void OnBnClickedNewvar();
  afx_msg void OnEnChangeEffSucceeded();
  afx_msg void OnCbnSelchangeOperator();
  afx_msg void OnCbnSelchangeNativeVariable();
